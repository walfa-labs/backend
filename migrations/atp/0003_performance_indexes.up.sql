-- 0003_performance_indexes.up.sql — performance indexes for foreign keys, joins, and composite filters.
...
CREATE INDEX idx_projects_status_sort ON projects (status, sort_order ASC, published_at DESC);

-- Index for experience sorting and display
CREATE INDEX idx_experiences_sort ON experiences (sort_order ASC, start_date DESC);

-- Foreign key lookup indexes
CREATE INDEX idx_project_links_project_id ON project_links (project_id);
CREATE INDEX idx_exp_highlights_exp_id ON experience_highlights (experience_id);
