...
// experienceColumns lists the experiences columns in scan order.
// "current" is also a reserved word in PostgreSQL and must stay double-quoted.
...
func scanExperience(row rowScanner) (*domain.Experience, error) {
	...
	// e.Current is scanned directly as bool — PostgreSQL native BOOLEAN
	return &e, nil
}
...
func (r *ExperienceRepo) Update(ctx context.Context, e *domain.Experience) error {
	...
	res, err := tx.ExecContext(ctx, `
		...
		e.EndDate, e.Current, e.SummaryMarkdown, e.SortOrder, e.ID.String(),
	)
	if err != nil {
		return err
	}
...
func (r *ExperienceRepo) replaceHighlights(ctx context.Context, tx *sql.Tx, experienceID uuid.UUID, highlights []domain.ExperienceHighlight) error {
	...
	for _, h := range highlights {
		_, err := tx.ExecContext(ctx, `
			...
			VALUES ($1, $2, $3, $4)`,
			uuid.New().String(), experienceID.String(), h.BodyMarkdown, h.SortOrder)
		if err != nil {
			return err
		}
	}
