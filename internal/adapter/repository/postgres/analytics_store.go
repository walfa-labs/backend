...
// AnalyticsStore implements port.AnalyticsStore against a PostgreSQL database.
// Unlike the Oracle adapter which uses a separate ADW instance, the PostgreSQL
// analytics store co-locates the star-schema (dim_posts + fact_post_views)
// in the same database as the OLTP data — suitable for development and
// smaller production deployments.
type AnalyticsStore struct {
	db *sql.DB
}

// NewAnalyticsStore constructs an AnalyticsStore bound to the given pool.
...
// single transaction. Uses INSERT ... ON CONFLICT instead of Oracle MERGE.
func (s *AnalyticsStore) RecordPostView(ctx context.Context, view port.PostView) error {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO dim_posts (post_id, slug, title)
		VALUES ($1, $2, $3)
		ON CONFLICT (post_id) DO UPDATE SET slug = EXCLUDED.slug, title = EXCLUDED.title`,
		view.PostID.String(), view.Slug, view.Title); err != nil {
		return fmt.Errorf("upsert dim_posts: %w", err)
	}
...
// Uses PostgreSQL DATE_TRUNC instead of Oracle TRUNC(col, unit).
func (s *AnalyticsStore) ViewsTimeSeries(ctx context.Context, from, to time.Time, bucket string) ([]port.ViewsBucket, error) {
	...
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`
		SELECT DATE_TRUNC('%s', viewed_at) AS b, COUNT(*)
		FROM fact_post_views
		WHERE viewed_at >= $1 AND viewed_at <= $2
		GROUP BY DATE_TRUNC('%s', viewed_at)
...
// truncUnits maps the public bucket name to a PostgreSQL DATE_TRUNC unit.
