...
// StatsRepo implements port.StatsRepo against PostgreSQL. It covers only the
// operational counts; view analytics (TotalPostViews, time series, top posts)
// live in the AnalyticsStore and are composed by the service layer.
...
// from the analytics store, the analytical source of truth for views.
// Uses correlated subqueries from VALUES(1) instead of Oracle's FROM DUAL.
func (r *StatsRepo) Summary(ctx context.Context) (port.StatsSummary, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT
		    ...
		    (SELECT COUNT(*) FROM projects WHERE featured = true AND status = 'published'),
		    (SELECT MIN(start_date) FROM experiences WHERE experience_type = 'work')
		FROM (VALUES(1)) AS t(x)`).Scan(
