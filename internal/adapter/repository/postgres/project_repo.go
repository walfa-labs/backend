...
func scanProject(row rowScanner) (*domain.Project, error) {
	...
	err := row.Scan(
		...
		&repoURL, &demoURL, &techStackJSON, &p.Status, &p.Featured, &p.SortOrder,
	...
	// p.Featured is scanned directly as bool — PostgreSQL native BOOLEAN
...
// techStackToJSON encodes the tech stack slice for the JSONB column.
...
func (r *ProjectRepo) Create(ctx context.Context, p *domain.Project) error {
	...
	_, err = tx.ExecContext(ctx, `
		...
		p.ID.String(), p.Slug, p.Title, p.Tagline, p.DescriptionMarkdown, p.CoverImageURL,
		p.RepoURL, p.DemoURL, techStack, string(p.Status), p.Featured, p.SortOrder, p.PublishedAt,
	)
	if err != nil {
...
func (r *ProjectRepo) Update(ctx context.Context, p *domain.Project) error {
	...
	res, err := tx.ExecContext(ctx, `
		UPDATE projects SET
		    ...
		    cover_image_url = $5, repo_url = $6, demo_url = $7, tech_stack = $8::jsonb,
		    status = $9, featured = $10, sort_order = $11, published_at = $12,
		    updated_at = NOW()
		WHERE project_id = $13`,
		p.Slug, p.Title, p.Tagline, p.DescriptionMarkdown, p.CoverImageURL,
		p.RepoURL, p.DemoURL, techStack, string(p.Status), p.Featured, p.SortOrder,
